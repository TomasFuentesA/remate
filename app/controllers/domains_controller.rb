class DomainsController < ApplicationController
        ...
        if member.length == 0
          ...
          @compradores.each do |comp|
            ...
            if acciones == 0
              ...
              porcentaje = ((acciones * 100).to_f/@legalpersona.acciones)
          ...
          acciones = domrol.acciones_venta + member[0].acciones.to_i
          porcentaje = ((acciones*100).to_f/@legalpersona.acciones)
          member[0].update(acciones: acciones, percentage: porcentaje.round(2))
          domrol.destroy
        end
      end
          ...
          if porcentaje != 100
            ...
            (0..params[:vendedor].length-1).each do |i|
              if params[:vendedor][i].chars[params[:vendedor][i].length - 1] == 'N'
                ...
                PersonaMember.create(legal_persona_id: @legalpersona.id, percentage: params[:percentage][i].to_f, type_member: "Natural", persona_id: params[:vendedor][i].split("N")[0].to_i, acciones: acciones.to_i, entrada: params[:date_posetion], capital_actual: -params[:price][i].to_i, valor: params[:price][i].to_i) 
                ...
                Rails.logger.info "Es legal"
                acciones = (params[:percentage][i].to_f * @legalpersona.acciones)/100
                PersonaMember.create(legal_persona_id: @legalpersona.id, percentage: params[:percentage][i].to_f, type_member: "Legal", persona_id: params[:vendedor][i].split("L")[0].to_i, acciones: acciones.to_i, entrada: params[:date_posetion], capital_actual: -params[:price][i].to_i, valor: params[:price][i].to_i)
                @domain = Domain.create(type_modality: params[:type_modality], price: acciones.to_i, date_posetion: params[:date_posetion], domainable_id: @legalpersona.id, domainable_type: 'LegalPersona', percentage: params[:percentage][i].to_f, notario_id: params[:domain][:notario_id], valor: params[:price][i].to_i) 
  ...
  def compra_venta
   begin
      ...
      (0..params[:vendedor].length-1).each do |i|
        if params[:vendedor][i].chars[params[:vendedor][i].length - 1] == 'N'
          ...
          if (params[:percentage][i].to_f <= 0) || (params[:percentage][i].to_i > 100)
            ...
            @eliminar = DomainRol.where(domain_id: @domain.id)
            if @eliminar.length != 0             
              @eliminar.each do |eliminar|
                member = PersonaMember.where(persona_id: eliminar.persona_id, type_member: eliminar.type_member, legal_persona_id: @domain.domainable_id)
                if member.length == 0
                  historico = Historico.where(persona_id: eliminar.persona_id,type_member: eliminar.type_member,  legal_persona_id: @domain.domainable_id)
                  porcentaje = (historico[0].acciones*100).to_f/@legal_persona.acciones
                  PersonaMember.create(legal_persona_id: @domain.domainable_id, type_member: domrol.type_member, persona_id: domrol.persona_id, acciones: historico[0].acciones, percentage: porcentaje.round(2), entrada: historico[0].entrada)
                  eliminar.destroy
                else
                  acciones = eliminar.acciones_venta + member[0].acciones.to_i
                  porcentaje = ((acciones*100).to_f/@legal_persona.acciones)
                  member[0].update(acciones: acciones, percentage: porcentaje.round(2))
                  eliminar.destroy
                end    
              end
            ...
            n_acciones = @personam[0].acciones - @domrol.acciones_venta
            if n_acciones <= 0
              Historico.create(persona_id: params[:vendedor][i].split("N")[0].to_i, type_member: "Natural", legal_persona_id: @legal_persona.id, salida: @domain.date_posetion, entrada: @personam[0].entrada, valor: params[:price][i].to_i, acciones: @personam[0].acciones*((@domrol.percentage/100).to_f))
              @personam[0].destroy         
            else
              porcentaje = (n_acciones*100).to_f/@legal_persona.acciones
              @personam[0].update(acciones: n_acciones, percentage: porcentaje.round(2))
            end
          end   
        else
          @personam = PersonaMember.where(persona_id: params[:vendedor][i].split("L")[0].to_i ,type_member: "Legal", legal_persona_id: @legal_persona.id)
          if (params[:percentage][i].to_f <= 0) || (params[:percentage][i].to_i > 100)
            ...
            Rails.logger.info @personam[0].acciones
            Rails.logger.info @domrol.acciones_venta
            
            n_acciones = @personam[0].acciones - @domrol.acciones_venta
            Rails.logger.info n_acciones 
            if n_acciones <= 0
              Historico.create(persona_id: params[:vendedor][i].split("L")[0].to_i, type_member: "Legal", legal_persona_id: @legal_persona.id, salida: @domain.date_posetion, entrada: @personam[0].entrada, valor: params[:price][i].to_i, acciones: @personam[0].acciones*((@domrol.percentage/100).to_f))
              @personam[0].destroy         
            else
              porcentaje = (n_acciones*100).to_f/@legal_persona.acciones
              @personam[0].update(acciones: n_acciones, percentage: porcentaje.round(2))
      ...
      if @domain.save
        flash[:notice] = "Compra y Venta creada con exito"
    ...
    begin
      ...
      if c.chars[c.length - 1] == "N"
        ...
        if member.length == 0
          ...
          PersonaMember.create(legal_persona_id: @legalpersona.id, percentage: porcentaje.to_f.round(2), type_member: "Natural", persona_id: c.chars[0].to_i, acciones: params[:price].to_i, entrada: params[:date_posetion], capital_actual: -params[:valor].to_i, valor: params[:valor].to_i)
          @actuales = PersonaMember.where(legal_persona_id: @legalpersona.id)
          @actuales.each do |actual|
            if actual.id != member[0].id
              nuevo_porcentaje = (actual.acciones * 100).to_f/@legalpersona.acciones
          ...
          member[0].update(acciones: nueva_accion, percentage: ((nueva_accion*100).to_f/@legalpersona.acciones).round(2), capital_actual: member[0].capital_actual - params[:valor].to_i)
          @actuales = PersonaMember.where(legal_persona_id: @legalpersona.id)
          @actuales.each do |actual|
            if actual.id != member[0].id
              nuevo_porcentaje = (actual.acciones * 100).to_f/@legalpersona.acciones
        ...
        if member.length == 0
          ...
          @legalpersona.update(acciones: (@legalpersona.acciones + params[:price].to_i), total: (@legalpersona.total + params[:valor].to_i ))
          porcentaje = (params[:price].to_i * 100).to_f/@legalpersona.acciones
          PersonaMember.create(legal_persona_id: @legalpersona.id, percentage: porcentaje.to_f.round(2), type_member: "Legal", persona_id: c.chars[0].to_i, acciones: params[:price].to_i, entrada: params[:date_posetion], capital_actual: -params[:valor].to_i , valor: params[:valor].to_i)
          @actuales = PersonaMember.where(legal_persona_id: @legalpersona.id)
          @actuales.each do |actual|
            nuevo_porcentaje = (actual.acciones * 100).to_f/@legalpersona.acciones
            actual.update(percentage: nuevo_porcentaje.round(2))
